...
export const Dashboard = () => {
  ...
  return <main className="max-w-6xl mx-auto px-6 py-10">
    ...
    <section className="bg-white/90 dark:bg-slate-900/90 rounded-2xl border border-white dark:border-slate-800 shadow-xl overflow-hidden">
      ...
      </div><div className="flex flex-wrap items-center gap-2 shrink-0 self-start"><button onClick={() => descargarInformeTriaje(t)} className="inline-flex h-9 items-center rounded-lg bg-teal-600 px-3 text-sm font-semibold leading-none text-white transition-colors hover:bg-teal-700">Descargar informe</button><button onClick={() => navigate('/triaje', { state: { triajeEditar: t } })} className="inline-flex h-9 items-center rounded-lg bg-blue-100 px-3 text-sm font-semibold leading-none text-blue-700 transition-colors hover:bg-blue-200">Editar</button><button onClick={() => void eliminar(t)} className="inline-flex h-9 items-center rounded-lg bg-red-100 px-3 text-sm font-semibold leading-none text-red-700 transition-colors hover:bg-red-200">Eliminar</button></div></div></article>)}</div>}
    </section>
  </main>;
};
