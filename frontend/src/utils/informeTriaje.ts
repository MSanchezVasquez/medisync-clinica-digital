...
export const descargarInformeTriaje = (t: DatosInformeTriaje) => {
  ...
  pdf.text(`Nombre: ${t.nombreCompleto}`, 16, 60);
  pdf.text(`DNI: ${t.dni || 'Sin registrar'}`, 16, 68);
  pdf.text(`Edad: ${t.edad} anos`, 16, 76);
  pdf.text(`Fecha: ${new Date(t.creadoEn).toLocaleString('es-PE')}`, 105, 60);
  pdf.text(`Peso: ${t.peso} kg`, 105, 68);
  pdf.text(`Altura: ${t.altura} cm`, 105, 76);
  ...
  y = seccion(
    ...
    `Urgencia: ${t.urgencia}\nEspecialidad sugerida: ${t.especialidad}`,
    y
  );
  ...
  pdf.text(
    'Este informe es orientativo y no sustituye una evaluacion medica profesional.',
    16,
    282
  );
